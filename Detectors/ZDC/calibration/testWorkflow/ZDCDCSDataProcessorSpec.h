 ...
 public:
  void init(o2::framework::InitContext& ic) final
  {
    ...
    if (mDPsUpdateInterval == 0) {
      LOG(ERROR) << "ZDC DPs update interval set to zero seconds --> changed to 60";
    ...
    if (useCCDBtoConfigure) {
      ...
      std::unordered_map<DPID, std::string>* dpid2DataDesc = mgr.getForTimeStamp<std::unordered_map<DPID, std::string>>("ZDC/DCSconfig", ts);
      for (auto& i : *dpid2DataDesc) {
        vect.push_back(i.first);
      }
    } else {
      LOG(INFO) << "Configuring via hardcoded strings";
      std::vector<std::string> aliases = {"zdc_pos[00..04]"};
      std::vector<std::string> aliasesInt = {"zdc_hv[00..22]", "zdc_hva[00..22]", "ZDC_CONFIG_[00..08]", "TOF_FEACSTATUS_[00..71]"};
